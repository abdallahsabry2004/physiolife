...
import { generatePDF } from "@/lib/pdfExport"; // استيراد دالة الـ PDF
...
function PatientDetail() {
  ...
  const updatePatientInfo = useMutation({
    mutationFn: async () => {
      const payload = {
        full_name: editForm.full_name, gender: editForm.gender || null, age: editForm.age ? Number(editForm.age) : null,
        phone: editForm.phone || null, diagnosis: editForm.diagnosis || null, referral_source: editForm.referral_source || null,
        occupation: editForm.occupation || null, address: editForm.address || null,
      };
  ...
  const deletePatient = useMutation({
    mutationFn: async () => {
      await supabase.rpc('delete_patient_completely', { p_id: id });
    },
  ...
  const newSession = useMutation({
    mutationFn: async () => {
      ...
      await supabase.from("treatment_sessions").insert({ patient_id: id, session_number: sessionNum, therapist_id: user?.id ?? null });
    },
  ...
  const updateSession = useMutation({
    mutationFn: async ({ sid, patch }: { sid: string; patch: Record<string, unknown> }) => {
      await supabase.from("treatment_sessions").update(patch as never).eq("id", sid);
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["sessions", id] }),
  });
  ...
  const deleteSession = useMutation({
    mutationFn: async (sessionId: string) => {
      await supabase.from("treatment_sessions").delete().eq("id", sessionId);
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["sessions", id] }),
  });

  const painSeries = [...sessions].reverse().filter((s) => s.pain_before !== null || s.pain_after !== null).map((s) => ({
    name: `#${s.session_number}`, before: s.pain_before, after: s.pain_after,
  }));

  const openEditModal = () => {
    if (patient) {
      setEditForm({
        full_name: patient.full_name || "", gender: patient.gender || "", age: patient.age || "", phone: patient.phone || "",
        diagnosis: patient.diagnosis || "", referral_source: patient.referral_source || "", occupation: patient.occupation || "", address: patient.address || "",
      });
      setEditOpen(true);
    }
  };

  // دالة تصدير الـ PDF الجديدة
  const handleExportPDF = async (mode: PrintMode) => {
    setPrintMode(mode);
    const toastId = toast.loading("Generating PDF document...");
    
    // ننتظر قليلاً حتى يتم عرض العناصر والرسوم البيانية بالكامل في الـ DOM
    setTimeout(async () => {
      try {
        await generatePDF('pdf-content', `${patient?.full_name}_${mode}_report.pdf`);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* حاوية التصدير للـ PDF (لا تعتمد على @media print إطلاقاً) */}
      {printMode && (
        <div id="pdf-content" className="bg-white p-8 w-[210mm] text-black mx-auto">
          <div className="border-b-2 border-primary pb-6 mb-6">
            ...
            <div className="mt-6 rounded-xl border-2 border-gray-200 p-4 break-inside-avoid">
              <div className="grid grid-cols-2 gap-y-4 gap-x-8 text-sm items-start">
                <div className="flex flex-col">
                  ...
                  <span className="font-bold text-lg">{patient.full_name}</span>
                </div>
                <div className="flex flex-col">
          ...
          <div className="space-y-8 w-full">
             ...
             {printMode === 'body' && (
                <div className="break-inside-avoid flex justify-center w-full">
                   <ProfessionalBodyChart patientId={id} sessionId={undefined} />
                </div>
             )}

             {printMode === 'progress' && (
                <div className="break-inside-avoid pdf-chart-container">
                   {painSeries.length > 0 && (
                      <ResponsiveContainer width="100%" height={400}>
                        <LineChart data={painSeries}>
                          <XAxis dataKey="name" stroke="#000" fontSize={12} />
                          <YAxis domain={[0, 10]} stroke="#000" fontSize={12} />
                          <Line type="monotone" dataKey="before" stroke="#008080" strokeWidth={3} />
                          <Line type="monotone" dataKey="after" stroke="#e11d48" strokeWidth={3} />
                        </LineChart>
                      </ResponsiveContainer>
                   )}
                </div>
          ...
          <div className="mt-12 pt-6 border-t border-gray-200 text-center text-sm text-gray-500 break-inside-avoid">
            ...
            <p className="text-xs mt-1">Official Medical Record</p>
          </div>
        </div>
      )}

      {/* الواجهة الرئيسية للموقع */}
      <div className={printMode ? "hidden" : "block"}>
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex gap-2 items-center">
            ...
            <Button variant="outline" onClick={() => handleExportPDF('full')}>
              <Printer className="mr-2 h-4 w-4" /> Export Full PDF
            </Button>
        ...
        <Tabs defaultValue="history">
          ...
          <TabsContent value="sessions" className="mt-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              {sessions.length > 0 && (
                <Button variant="outline" onClick={() => handleExportPDF('sessions')} className="ml-auto">
                  <Printer className="mr-2 h-4 w-4" /> Export Sessions PDF
                </Button>
              )}
            </div>
            {sessions.length === 0 && (
          ...
          <TabsContent value="body" className="mt-6 space-y-4">
            <div className="flex justify-end mb-2">
               <Button variant="outline" onClick={() => handleExportPDF('body')} size="sm">
                  <Printer className="mr-2 h-4 w-4" /> Export Body Chart PDF
               </Button>
            </div>
            <ProfessionalBodyChart patientId={id} sessionId={undefined} />
          </TabsContent>
          ...
          <TabsContent value="progress" className="mt-6">
            <Card>
              <CardHeader className="flex flex-row justify-between items-center">
                ...
                {painSeries.length > 0 && (
                  <Button variant="outline" onClick={() => handleExportPDF('progress')} size="sm">
                    <Printer className="mr-2 h-4 w-4" /> Export Progress PDF
                  </Button>
                )}
              </CardHeader>
          ...
          <TabsContent value="measures" className="mt-6">
            <div className="flex justify-end mb-4">
               <Button variant="outline" onClick={() => handleExportPDF('measures')} size="sm">
                  <Printer className="mr-2 h-4 w-4" /> Export Measurements PDF
               </Button>
            </div>
            <PatientMeasurements patientId={id} />
          </TabsContent>
          ...
          <TabsContent value="questionnaires" className="mt-6">
            <div className="flex justify-end mb-4">
               <Button variant="outline" onClick={() => handleExportPDF('questionnaires')} size="sm">
                  <Printer className="mr-2 h-4 w-4" /> Export Questionnaires PDF
               </Button>
            </div>
            <PatientAssessments patientId={id} />
          </TabsContent>
